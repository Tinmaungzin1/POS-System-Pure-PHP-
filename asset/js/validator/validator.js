...
(function (root, factory) {
  ...
  else if (typeof exports === "object") exports["FormValidator"] = factory();
  else root.FormValidator = factory();
})(this, function () {
  ...
  FormValidator.prototype = {
    ...
    tests: {
      sameAsPlaceholder: function (field, data) {
        if (field.getAttribute("placeholder"))
          return (
            data.value != field.getAttribute("placeholder") || this.texts.empty
          );
        else return true;
      },
      ...
      email: function (field, data) {
        if (
          !this.settings.regex.email.filter.test(data.value) ||
      ...
      text: function (field, data) {
        ...
        if (data.validateWords) {
          ...
          var wordsLength = function (len) {
            for (var w = words.length; w--; )
              if (words[w].length < len) return that.texts.short;
            return true;
          };
        ...
        if (
          data.lengthRange &&
          data.lengthRange[1] &&
          data.value.length > data.lengthRange[1]
        ) {
      ...
      time: function (field, data) {
        ...
        if (pattern.test(data.value)) return true;
        else return this.texts.time;
      },
    ...
    events: function (types, formElm) {
      ...
      if (types instanceof Array) types.forEach(bindEventByType);
      else if (typeof types == "string") bindEventByType(types);
    ...
    prepareFieldData: function (field) {
      ...
      // Special treatment
      if (nodeName === "select") this.data[id].type = "select";
      else if (nodeName === "textarea") this.data[id].type = "text";

      /* Gather Custom data attributes for specific validation:
       */
      this.data[id].validateWords =
        field.getAttribute("data-validate-words") || 0;
      this.data[id].lengthRange = field.getAttribute(
        "data-validate-length-range"
      )
        ? (field.getAttribute("data-validate-length-range") + "").split(",")
        : [1];
      this.data[id].lengthLimit = field.getAttribute("data-validate-length")
        ? (field.getAttribute("data-validate-length") + "").split(",")
        : false;
      this.data[id].minmax = field.getAttribute("data-validate-minmax")
        ? (field.getAttribute("data-validate-minmax") + "").split(",")
        : false; // for type 'number', defines the minimum and/or maximum for the value as a number.
    ...
    checkField: function (field, silent) {
      ...
      if (testResult !== true) data.valid = false;
      ...
      if (data.valid && data.validateLinked) {
        if (data["validateLinked"].indexOf("#") == 0)
          ...
          linkedTo = form.querySelector(
            "[name=" + data["validateLinked"] + "]"
          );
        else
          linkedTo = document.body.querySelector(
            "[name=" + data["validateLinked"] + "]"
          );

        testResult = this.tests.linked.call(
          this,
          field.value,
          linkedTo.value,
          data.type
